...
namespace :test do
  ...
  task :ios do
    $ios_success = system("set -o pipefail && xcodebuild clean test -scheme 'Tests-iOS' -workspace #{workspace_file} -destination 'name=iPhone 5s' | xcpretty -tc")
  end

  desc "Run Transporter Tests for Mac OS X"
  task :osx do
    $osx_success = system("set -o pipefail && xcodebuild clean test -scheme 'Tests-OSX' -workspace #{workspace_file} | xcpretty -tc")
  end
end
